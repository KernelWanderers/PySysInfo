# Credits to @[DhinakG](https://github.com/DhinakG) for allowing us to copy over their `ioreg.py` abstraction implementation from OpenCore-Legacy-Patcher:
...
def IORegistryEntryGetParentEntry(
    ...
    plane: io_name_t,
    parent: pointer
) -> tuple[kern_return_t, io_registry_entry_t]:
    raise NotImplementedError
...
def IORegistryEntryGetPath(
    ...
    plane: io_name_t,
    path: pointer
) -> tuple[kern_return_t, io_string_t]:
    raise NotImplementedError
...
def IOServiceGetMatchingServices(
    ...
    matching: dict,
    existing: pointer
) -> tuple[kern_return_t, io_iterator_t]:
    raise NotImplementedError
